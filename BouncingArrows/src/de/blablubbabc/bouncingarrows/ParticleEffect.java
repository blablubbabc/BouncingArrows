...
import de.blablubbabc.bouncingarrows.ReflectionHandler.PackageType;
import de.blablubbabc.bouncingarrows.ReflectionHandler.PacketType;
import de.blablubbabc.bouncingarrows.ReflectionHandler.SubPackageType;
...
public enum ParticleEffect {
	...
	static {
		...
		try {
			packetPlayOutWorldParticles = ReflectionHandler.getConstructor(PacketType.PLAY_OUT_WORLD_PARTICLES.getPacket(), String.class, float.class, float.class, float.class, float.class,
	...
	public static ParticleEffect fromName(String name) {
		if (name != null) for (Entry<String, ParticleEffect> e : NAME_MAP.entrySet())
	...
	private static List<Player> getPlayers(Location center, double range) {
		...
		for (Player p : Bukkit.getOnlinePlayers())
			if (p.getWorld().getName().equals(name) && p.getLocation().distanceSquared(center) <= squared) players.add(p);
		return players;
	}
	...
	private static Object instantiatePacket(String name, Location center, float offsetX, float offsetY, float offsetZ, float speed, int amount) {
		if (amount < 1) throw new PacketInstantiationException("Amount cannot be lower than 1");
		try {
	...
	public void display(Location center, double range, float offsetX, float offsetY, float offsetZ, float speed, int amount) {
		if (range > MAX_RANGE) throw new IllegalArgumentException("Range cannot exceed the maximum value of 16");
	...
	public static void displayIconCrack(Location center, double range, int id, float offsetX, float offsetY, float offsetZ, float speed, int amount) {
		if (range > MAX_RANGE) throw new IllegalArgumentException("Range has to be lower/equal the maximum of 16");
